...
import { Cdata, ERC1155Abi, ERC1155Add, ERC20, IERC20, MarketAbi, MarketAdd, TokenAbi, TokenAdd } from "../../config";
...
const Upload2 = () => {
  ...
  const tokenContract = getContract(library, account,ERC1155Add,ERC1155Abi);
...
const upLoadNFT = async ()=>{
  ...
  console.log("video",tokenContract) 
  ...
  // const tx = await tokenContract.mint(account,numberOfNFT, jsonIPFS.path,

  //   {gasLimit:3000000})

  const tx = await tokenContract.MintW(account,numberOfNFT,jsonIPFS.path,{gasLimit:3000000})
  await tx.wait()
...
const approval = async ()=>{
  ...
  try {
    const tx = await tokenContract.setApprovalForAll(MarketAdd,true)
    await tx.wait()
...
setLockLoader(true)
  try {
    ...
    const tx = await marketContract.createAuction115(
        Counter,Date.parse(AuctionEnd)/1000,utils.parseUnits(PriceinEth.toString(),"ether") ,ERC1155Add
    ,    [
...
const Lock3 = async ()=>{
  setLockLoader(true)
    try {
      ...
      const trvOff = travelOffer ? 1 : 0
      const playAble = play? 1: 0
      const tx = await marketContract.createSale1155(
